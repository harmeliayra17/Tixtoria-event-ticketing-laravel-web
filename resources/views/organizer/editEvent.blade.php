...
<main class="w-full pb-12">
  <div class="bg-white border border-slate-100 rounded-2xl p-6 md:p-8 shadow-sm">
    <h2 class="text-xl font-bold text-[#1B1464] border-b border-slate-100 pb-4 mb-6">Edit Event</h2>
    
    <form action="{{ route('organizer.updateEvent', $event->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-8">
      @csrf
      @method('PUT')
