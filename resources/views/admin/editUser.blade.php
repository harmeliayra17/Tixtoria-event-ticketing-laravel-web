...
<main class="w-full pb-12 animate-fade-in">
  <div class="bg-white border border-slate-100 rounded-2xl p-6 md:p-8 shadow-sm">
    <h2 class="text-xl font-bold text-[#1B1464] border-b border-slate-100 pb-4 mb-6">Edit User Settings</h2>
    
    <form action="{{ route('admin.updateUser', $user->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-8" autocomplete="off">
      @csrf
      @method('PUT')
      ...
      <div class="flex flex-col items-center space-y-4">
        ...
        <p class="text-[11px] text-slate-400 text-center">Click the picture to change photo.</p>
        @error('profile')
