...
<main class="w-full pb-12 animate-fade-in">
  <div class="bg-white border border-slate-100 rounded-2xl p-6 md:p-8 shadow-sm">
    <h2 class="text-xl font-bold text-[#1B1464] border-b border-slate-100 pb-4 mb-6">Edit Event Details</h2>
    
    <form action="{{ route('admin.updateEvent', $event->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-8">
      @csrf
      @method('PUT')
      ...
      <div class="flex flex-col items-center space-y-4">
        ...
        <p class="text-[11px] text-slate-400 text-center">Click the container to change banner image.</p>
        @error('image')
      ...
      <div class="md:col-span-2 space-y-5">
          ...
          <div class="relative">
            <select id="category" name="id_category" 
              ...
              @foreach ($categories as $category)
                <option value="{{ $category->id }}" {{ old('id_category', $event->id_category) == $category->id ? 'selected' : '' }}>
                  {{ $category->name }}
                </option>
              @endforeach
            </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" id="city" name="city" value="{{ old('city', $event->location->city) }}"
              class="w-full h-11 bg-slate-50 border border-slate-200 rounded-xl px-4 focus:outline-none focus:border-[#640D5F] text-sm"
              required>
          </div>
          <div>
            <label for="province" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Province</label>
            <input type="text" id="province" name="province" value="{{ old('province', $event->location->province) }}"
              class="w-full h-11 bg-slate-50 border border-slate-200 rounded-xl px-4 focus:outline-none focus:border-[#640D5F] text-sm"
              required>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" id="price" name="price" value="{{ old('price', $event->price) }}" placeholder="0 for Free"
              class="w-full h-11 bg-slate-50 border border-slate-200 rounded-xl px-4 focus:outline-none focus:border-[#640D5F] text-sm">
          </div>
          <div>
